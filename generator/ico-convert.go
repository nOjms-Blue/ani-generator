...
func ConvertImageDataToIcon(image loader.ImageData, resourceType ResourceType, hotSpotX int16, hotSpotY int16) ([]byte, error) {
	...
	expectedPixels := image.Width * image.Height
	if int64(len(image.Pixels)) != expectedPixels {
		return nil, fmt.Errorf("invalid pixel count: got %d, want %d", len(image.Pixels), expectedPixels)
	}
	if resourceType == CursorResource && (hotSpotX < 0 || int64(hotSpotX) >= image.Width || hotSpotY < 0 || int64(hotSpotY) >= image.Height) {
		return nil, errors.New("cursor hot spot is outside the image")
	}
